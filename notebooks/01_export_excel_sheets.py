...
if not excel_path.exists():
    print("File not found! Confirm path.")
...
def convert_to_percentage(df, theme_cols):
    ...
    for col in theme_cols:
        ...
        # If values are between 0 and 1, convert to percentages
        ...
        # If values are something else (maybe counts), warn but keep as is
            ...
            print(f"'{col}' has unusual range: {min_val:.2f}-{max_val:.2f} (not converted)")
    
    return df_converted


for sheet_name in scope:
    if sheet_name not in all_sheets:
        print(f"'{sheet_name}' not found in the file")
        continue
    ...
    try:
        ...
        if theme_cols:
            ...
            df_converted = df
            print("No numeric theme columns found")
        ...
        print(f" Error found in '{sheet_name}': {e}")
...
print("Part 1 completed. Proceeding with heatmap graphing")
